...
<head>
    ...
    <title>Contacts | Musumba Steel</title>
...
<body>
    <div class="admin-header">
        <div class="user-info">
            <div>
                ...
                <p style="margin: 0; opacity: 0.9;"><?= t('admin.manage_contacts'); ?></p>
            </div>
        ...
        <nav class="admin-nav">
            ...
            <a href="contacts.php?lang=<?= $lang; ?>" style="background: var(--primary); color: #111;"><?= t('admin.contacts'); ?></a>
    ...
    <div class="admin-layout">
                ...
                <?php foreach ($contacts as $person): ?>
                    <tr>
                        <td><?= htmlspecialchars($person['name']); ?></td>
                                <td><?= htmlspecialchars(ucfirst(str_replace('-', ' ', $person['department']))); ?></td>
                        <td><?= htmlspecialchars($person['email']); ?></td>
                        <td><?= htmlspecialchars($person['phone']); ?></td>
                                <td><a href="?lang=<?= $lang; ?>&id=<?= $person['id']; ?>"><?= t('admin.edit'); ?></a></td>
                    </tr>
                <?php endforeach; ?>
                    <?php endif; ?>
            </tbody>
        ...
        <section class="dashboard-section">
            <h2><?= $editing ? t('admin.edit_contact') : t('admin.new_contact'); ?></h2>
        <form method="post">
            ...
            <label>
                    ...
                    <option value="general-management" <?= (($editing['department'] ?? '') === 'general-management') ? 'selected' : ''; ?>>General Management</option>
                    <option value="sales-management" <?= (($editing['department'] ?? '') === 'sales-management') ? 'selected' : ''; ?>>Sales Management</option>
                </select>
            </label>
            <label>
                        <?= t('admin.full_name'); ?>
            ...
            <label>
                        <?= t('admin.position'); ?>
                        <input type="text" name="position" value="<?= htmlspecialchars($editing['position'] ?? ''); ?>" required>
            </label>
            <label>
                <?= t('admin.email'); ?>
            ...
            <label>
                        <?= t('admin.phone'); ?>
                        <input type="text" name="phone" value="<?= htmlspecialchars($editing['phone'] ?? ''); ?>" required>
            </label>
            <label>
                        <?= t('admin.priority'); ?>
                <input type="number" name="priority" min="1" value="<?= htmlspecialchars($editing['priority'] ?? 1); ?>">
            </label>
                </div>
